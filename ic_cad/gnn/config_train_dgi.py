...
def parse_args():
    ...
    # 模型架構
    p.add_argument('--hidden-dim', type=int, default=64, help='GAT 隱藏維度')  # 調回較保守的值
    p.add_argument('--num-layers', type=int, default=2, help='GAT 層數')  # 先用 2 層
    p.add_argument('--dropout', type=float, default=0.2, help='GAT Dropout')
    p.add_argument('--cell-embed-dim', type=int, default=16, help='cell embedding 維度')  # 保持較小的 embedding
    p.add_argument('--heads-schedule', type=str, default='2,1', help='每層 head 數，如 "2,1"；空字串用預設')  # 保守的多頭配置

    # GRACE 超參
    p.add_argument('--tau', type=float, default=0.2, help='InfoNCE 溫度參數')  # 回到穩定值
    p.add_argument('--feat-mask', type=float, default=0.15, help='特徵隨機遮蔽比例（不遮最後一維 cell_id）')  # 適中的增廣
    p.add_argument('--edge-drop', type=float, default=0.15, help='邊隨機丟棄比例')  # 適中的結構噪聲
    p.add_argument('--use-proj', action='store_true', help='啟用 projection head（2-layer MLP）')
    p.add_argument('--proj-dim', type=int, default=64, help='projection head 輸出維度')  # 與 hidden_dim 一致

    # 訓練
    p.add_argument('--epochs', type=int, default=250)  # 適中的訓練輪數
    p.add_argument('--lr', type=float, default=5e-4)  # 更保守的學習率
    p.add_argument('--weight-decay', type=float, default=1e-4)  # 適中的正則化
    p.add_argument('--batch-size', type=int, default=2)  # 較小的批次
    p.add_argument('--gradient-accumulation', type=int, default=4)  # 增加梯度累積補償小批次
    p.add_argument('--grad-clip', type=float, default=1.0)
    p.add_argument('--warmup-epochs', type=int, default=10, help='學習率預熱 epochs')

    # 對比損失計算參數（省顯存）
    p.add_argument('--nce-chunk', type=int, default=4096, help='InfoNCE 分塊大小（行數）')
    ...
    p.add_argument('--test-only', action='store_true', help='僅用 c17，快速測試')
...
def nt_xent_loss_chunked(z1, z2, tau=0.2, chunk=4096):
    # L2 normalize
...
def train_grace(encoder, proj, loader, optimizer, scheduler, config, device, tag="", probe_data=None):
    ...
    for epoch in range(config.epochs):
        ...
        for step, data in enumerate(loader, start=1):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp, dtype=torch.bfloat16):
                ...
                if proj is not None:
                    z1 = proj(z1)
                    z2 = proj(z2)
                loss = nt_xent_loss_chunked(z1, z2, tau=config.tau, chunk=config.nce_chunk) / accum_steps
            ...
            del data, x1, e1, x2, e2, z1, z2, loss
        ...
        if ((epoch + 1) % config.probe_every == 0) or (epoch == config.epochs - 1):
            ...
            auc = link_pred_probe_auc(encoder, probe_batch, device, num_samples=config.probe_samples,
